...
def make_videos(df, im_path, vid_path, vmode=False):
    ...
    for c_row in df.index:
        ...
        vt.vid_from_seq(image_path, vid_outpath, frStart=fr_start, frEnd=fr_end, fps=fps, imQuality=imquality,
                        prefix=prefix, nDigits=ndigits, inSuffix=suffix_in, downSample=downsample, vertPix=vertpix,
                        roi=r, vMode=vmode)
...
def convert_videos(df, in_path, out_path, vmode=True, vertpix=None, imquality=0.35, suffix_in='MOV', suffix_out='mp4'):
    ...
    for c_row in df.index:
        ...
        in_path = in_path + os.path.sep + df.video_filename[c_row] + '.' + suffix_in

        out_path = out_path + os.path.sep + os.path.splitext(os.path.basename(in_path))[0] + '.' + suffix_out

        # Create movie
        vt.vid_convert(in_path, out_path, imQuality=imquality, vertPix=vertpix,
                       roi=r, vMode=vmode)
